...
function checkAlreadyInstalled() {
  if [[ -f $PREFIX/etc/apt/sources.list.d/bhutuuyt.repo.list ]]; then
...
function installRepository() {
  ...
  echo "deb [trusted=yes] https://bhutuu.github.io/bhutuuyt.repo/ bhutuuyt main" >$PREFIX/etc/apt/sources.list.d/bhutuuyt.repo.list
  rm -rf bhutuu.key >/dev/null 2>&1
  wget -q https://raw.githubusercontent.com/BHUTUU/bhutuuyt.repo/main/bhutuu.key
  apt-key add bhutuu.key
  cp -r $PREFIX/etc/apt/trusted.gpg~ $PREFIX/etc/apt/trusted.gpg.d/bhutuuyt.gpg
  apt-get update -yq --silent
  printf "Installation successful\n"
}
function removeRepository() {
  rm -rf $PREFIX/etc/apt/sources.list.d/bhutuuyt.repo.list >/dev/null 2>&1
